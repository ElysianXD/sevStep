...
func (a *IoctlAPI) register() error {
    ...
    // 验证结构体大小是否匹配（调试用）
    if C.sizeof_userspace_ctx_t != unsafe.Sizeof(registerStruct) {
        return fmt.Errorf("结构体大小不匹配: C=%d, Go=%d", 
            C.sizeof_userspace_ctx_t, unsafe.Sizeof(registerStruct))
    }
    
    // 检查文件描述符
    if a.kvmFile == nil {
        return fmt.Errorf("kvmFile is nil")
    }
    fd := a.kvmFile.Fd()
    if fd < 0 {
    ...
    _, _, errno := syscall.Syscall(
        syscall.SYS_IOCTL,
        fd,
        uintptr(C.KVM_USPT_REGISTER_PID),
        uintptr(unsafe.Pointer(&registerStruct)),
    )
    
    if errno != 0 {
        return fmt.Errorf("REGISTER_PID ioctl failed: %v", syscall.Errno(errno))
    }
    return nil
}
